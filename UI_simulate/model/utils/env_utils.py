import re
import math
from model.environments.environment_abstract import Environment
...
def get_environment(env_name: str) -> Environment:
    ...
    if env_name == 'cube3':
        from model.environments.cube3 import Cube3
        env = Cube3()
    elif puzzle_n_regex is not None:
        from model.environments.n_puzzle import NPuzzle
        puzzle_dim: int = int(math.sqrt(int(puzzle_n_regex.group(1)) + 1))
        env = NPuzzle(puzzle_dim)
    elif 'lightsout' in env_name:
        from model.environments.lights_out import LightsOut
